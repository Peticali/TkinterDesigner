from tkinter import Frame,Canvas,Tk,Scrollbar,Button,Label,END,INSERT,Listbox,Menu,IntVar,BOTTOM,Radiobutton,Entry,Text,LEFT,RIGHT
...
class app(): #NOT DELETE ANY COMMENTS GENERATED BY THE PROGRAM."""
    ...
    def resizeWindow(self,event):
        self.countdebug = self.countdebug + 1
    ...
    def Copy(self,e): #ainda precisa arrumar
        ...
        if newObjType == 'Button':
            self.objs[newObjName] = Button(self.frameDesigner,text='button')
        if newObjType == "Entry":
            self.objs[newObjName] = Entry(self.frameDesigner)
        if newObjType == "Frame":
            self.objs[newObjName] = Frame(self.frameDesigner)
        if newObjType == "Listbox":
            self.objs[newObjName] = Listbox(self.frameDesigner)
        if newObjType == 'Label':
            self.objs[newObjName] = Label(self.frameDesigner,text='Label')
    ...
    def __init__(self):
        ...
        self.applyButton = Button(self.janela,styleButtons,text="apply",command=self.apply)
        self.applyButton.place(x=576,y=390)

        self.exportButton = Button(self.janela,styleButtons,text="export",fg="#3BB143",command=self.getObjectsExport)
        self.exportButton.place(x=640,y=25,width=70)

        self.deleteButton = Button(self.janela,styleButtons,text="delete",fg="#FF0000",command=self.deleteComponent)
        self.deleteButton.place(x=640,y=55,width=70)

        self.RenameButton = Button(self.janela,styleButtons,text="rename",fg="#FFFFFF",command=self.renameWidgetWindow)
        self.RenameButton.place(x=640,y=85,width=70)

        self.CopyButton = Button(self.janela,styleButtons,text="copy",fg="#FFFFFF",command=lambda:[app.Copy(self,e=None)])
        self.CopyButton.place(x=640,y=115,width=70)
       
        self.detach = Button(text="detach",command=self.propertyDetach,bg='#18191A',borderwidth=0,fg='white')
        self.detach.place(x=640,y=150)

        self.janela.bind("<Configure>", self.resizeWindow)
        ...
        self.janela.bind('<Control-c>', self.Copy)
